...
fn = function (a, b) {return '<td title="'+_this.n+'DatePicker" class="strong cursorhand noborder" onclick="'+_this.n+'.update('+a+')">'+b+'</td>'},
...
for (var i=0; i<week.length; i++) con.push('<td title="'+this.n+'DatePicker" class="week">'+week[i]+'</td>');
...
_html += '<tr>'+fn("-1, null", "<<")+fn("null, -1", "<")+'<td title="'+this.n+'DatePicker" colspan=3 class="noborder">'+year+'-'+month+'-'+date+'</td>'+fn("null, 1", ">")+fn("1, null", ">>")+'</tr>';
...
createBox : function (html) {
	...
	box.className = this.config.className || 'datepicker';// || "date-picker-wp-div";
	
	//box.className = "date-picker-wp-div";
