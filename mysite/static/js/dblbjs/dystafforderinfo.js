...
{
    //向后台发送服务请求和数据
    $.post("json/staffOrderInfo!findDyAll.action",
        {
            optionCode:"getAll"
        },
        function(data){
            ...
            // 创建分页
            var dataList=$(data.planOrderDetailList);
...
function tableBuild(i,value){
    ...
    if(value.goodsType==0){
    	value.goodsType="飞机";
    }
    else if(value.goodsType==1){
    	value.goodsType="陆战装备";
    }
    ...
    txtHtml +="<td>"+isnull(value.remark)+"</td>";
    txtHtml +="</tr>";
