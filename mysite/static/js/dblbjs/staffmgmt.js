...
{
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!findAll.action",
        {
            optionCode:"getAll"
        },
        function(data){
            ...
            // 创建分页
            var dataList=$(data.staffMgmtList);
...
function OnSearch() {
    ...
    //根据查找条件得到Item
    var item = ""
    if($("#btncondition").text()=="员工编号"){
        item="staffId";
    }
    else if($("#btncondition").text()=="姓名"){
        item="staffName";
    }
    else if($("#btncondition").text()=="性别"){
        item="staffSex";
    }
    else if($("#btncondition").text()=="籍贯"){
        item="birthArea";
    }
    else if($("#btncondition").text()=="出生日期"){
        item="birthday";
    }
    else if($("#btncondition").text()=="工龄"){
        item="workAge";
    }

    //向后台发送服务请求和数据
    $.post("json/staffMgmt!findItems.action",
        {
            optionCode:"getAll",
            item:item,
            value:value
...
function tableBuild(i,value){
    ...
    txtHtml +="<td>"+value.staffId+"</td>";
    txtHtml +="<td>"+value.staffName+"</td>";
    txtHtml +="<td>"+value.staffSex+"</td>";
    txtHtml +="<td>"+value.birthArea+"</td>";
    txtHtml +="<td>"+value.birthday+"</td>";
    txtHtml +="<td>"+value.workAge+"</td>";
    txtHtml +="<td id='"+value.id+"'>";
    txtHtml +="<a class='glyphicon glyphicon-pencil' style='margin-left:10px;' title='修改' href='#' onclick='OnUpdate("+ value.id +")'></a>";
    txtHtml +="<a class='glyphicon glyphicon-remove' style='margin-left:10px;' title='删除' href='#' onclick='OnDeleteOne("+ value.id +")'></a>";
    txtHtml +="<a class='glyphicon glyphicon-search' style='margin-left:10px;' title='查看' href='#' onclick='OnView("+ value.id +")'></a>";
    txtHtml +="</td>";
    txtHtml +="</tr>";
...
function OnDeleteOne(recId)
{
    ...
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!delete.action",
        {
            optionCode:"delone",
            id:recId
        },
...
function OnDelete()
{
    ...
    //执行删除（多条记录）
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!deleteItems.action",
        {
...
function OnView(recId)
{
    ...
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!findItemById.action",
        {
            optionCode:"view",
            id:recId
        },
        function(data){
            if(data.actionStatus == "ok"){
                $("#staffId").val(data.staffMgmt.staffId);
                $("#staffName").val(data.staffMgmt.staffName);
                $("#staffSex").val(data.staffMgmt.staffSex);
                $("#birthArea").val(data.staffMgmt.birthArea);
                $("#birthday").val(data.staffMgmt.birthday);
                $("#workAge").val(data.staffMgmt.workAge);
                $("#modaldialog").modal("show");
            }
            else{
                alert(data.msg);
...
function OnSave()
{
    ...
    if(!isValidInteger($("#birthArea").val())){
    	alert("籍贯必须是整数");
    	$("#birthArea").focus();
    	return;
    }
...
{
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!save.action",
        {
            optionCode:"addnew",
            staffId:$("#staffId").val(),
            staffName:$("#staffName").val(),
            staffSex:$("#staffSex").val(),
            birthArea:$("#birthArea").val(),
...
{
    //向后台发送服务请求和数据
    $.post("json/staffMgmt!update.action",
        {
            optionCode:"update",
            id:gCurId,
            staffId:$("#staffId").val(),
...
function setControlsStatus(workMode)
{
	...
	if(workMode == WORK_MODE_ADD){
		...
		$("#staffSex").attr("disabled",false);
		$("#birthArea").attr("disabled",false);
		$("#birthday").attr("disabled",false);
		$("#workAge").attr("disabled",false);
        $("#btnsave").text("保存");
		$("#modaltitle").text("添加员工信息");
	}
		...
		$("#workAge").attr("disabled",false);
        $("#btnsave").text("保存");
        $("#modaltitle").text("修改员工信息");
    }
    else if(workMode == WORK_MODE_VIEW){
		$("#staffId").attr("disabled",true);
		$("#staffName").attr("disabled",true);
		$("#staffSex").attr("disabled",true);
		$("#birthArea").attr("disabled",true);
		$("#birthday").attr("disabled",true);
		$("#workAge").attr("disabled",true);
        $("#btnsave").text("确认");
        $("#modaltitle").text("查看员工信息");
    }
}
