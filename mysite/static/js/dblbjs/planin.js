...
//    $(document).on("click","#btndel",function(){alert(1);});
  
    
    $("#OorderId").change(function(e){
//    	alert($("#tblist_body").html());
    	var tb=$("#tblist_body").html();
    	if(tb!=""){
    		if(window.confirm("确认修改吗?将清空所有扫码信息")){
    			$("#tblist_body").html("");
    		}
    	}
    	var oid=$("#OorderId option:selected").val();
//    	alert(oid);
    	$.post("json/orderList!findTypeByOrderId.action",{orderId:oid},function(data){
    		if((data.orderList.taskType)=="0"){  
//    			$("#btnconfirm").val("确认入库");
    			str='<button type="button" id="btndel" class="btn btn-primary" data-toggle="modal" data-keyboard="false" data-backdrop="static" onclick="OnScanCode();">扫码</button><button type="button" id="btnconfirm" class="btn btn-primary" data-toggle="modal" data-keyboard="false" data-backdrop="static" onclick="OnCabinIn();">确认入库</button>';
    			tid=0;
    			$("#tt").html(str);
    		}
    		else if((data.orderList.taskType)=="1"){
//    			$("#btnconfirm").val("确认出库");
    			str='<button type="button" id="btndel" class="btn btn-primary" data-toggle="modal" data-keyboard="false" data-backdrop="static" onclick="OnScanCode();">扫码</button><button type="button" id="btnconfirm" class="btn btn-primary" data-toggle="modal" data-keyboard="false" data-backdrop="static" onclick="OnCabinIn();">确认出库</button>';
        		tid=1;	
    			$("#tt").html(str);
    		}
    	});
...
function SearchDyList(){
    ...
    $.post("json/dyList!findAllWithOrder.action",
        ...
        function(data){
			...
			for(var i=0; i<data.dyListList.length; i++){
				if(arr.indexOf(data.dyListList[i].orderId)<0){
				txtHtml +="<option  value='" + data.dyListList[i].orderId +"'>" + data.dyListList[i].orderId +"</option>";	
				arr.push(data.dyListList[i].orderId);
			}
			}	
			$("#OorderId").append(txtHtml);
...
function SearchByBoxCode(strCode){
    ...
    $.post("json/planInOut!findItemByBoxCodeAndOrder.action",
        {
            boxCode: strCode,
			planId:gPlanId,
			toOrderIds:orderid,
        ...
        function (data) {
            ...
            if (!bSameSpecIn) {		//Add new row
				...
				//名称有误
				if(actionStatus == "dy"){
					txtHtml +="<td style='color:red;'>"+data.dyList.dyId+"</td>";					
				}
				else{
				...
				//仓库不对
				if(actionStatus == "depot"){
					txtHtml +="<td style='color:red;'>"+data.dyList.depotId+"</td>";					
				}
				else{
...
//	alert(tid);
	if(tid==0){
	$.post("json/planInOut!inCabinOneByOne.action",
		{
			optionCode:"inCabinOneByOne",
			param:jsonParam
		},
		function(data){
    ...
    //执行入库（多条记录）
    //向后台发送服务请求和数据
    $.post("json/dyList!inCabinItems.action",
        {
            optionCode:"inCabinItems",
            toIncabinIds:toIncabinIds
        },
        function(data){
            if(data.actionStatus!="ok"){
                alert(data.msg);
...
function Update()
{
	...
	var rows = $("#tblist_body").find("tr");
	var tds = rows.eq(iRowNum).children();
