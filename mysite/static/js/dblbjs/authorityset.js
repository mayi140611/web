...
{
    //向后台发送服务请求和数据
    $.post("json/authoritySet!findAll.action",
        {
            optionCode:"getAll"
        },
        function(data){
            ...
            // 创建分页
            var dataList=$(data.authoritySetList);
            ...
            $.post("json/roleSet!findAll.action",
                    ...
                    function(data){
                    	 var dataList1=$(data.roleSetList);
                    	
                    	 $("#authority").empty();
                    	 $.each(dataList1,function(){ 
                    		 var html1=""; 
                    		html1+="<option>"+this.roleName+"</option>";
                    		$("#authority").append(html1);
                    	 });
                    });
     });
}
...
function OnSearch() {
    ...
    //根据查找条件得到Item
    var item = ""
    if($("#btncondition").text()=="用户名"){
        item="userName";
    }

    else if($("#btncondition").text()=="初始密码"){
        item="password";
    }
    else if($("#btncondition").text()=="当前权限"){
        item="authority";
    }

    //向后台发送服务请求和数据
    $.post("json/authoritySet!findItems.action",
        {
            optionCode:"getAll",
            item:item,
            value:value
...
function tableBuild(i,value){
    ...
    txtHtml +="<td>"+value.userName+"</td>";
   
    txtHtml +="<td>"+value.password+"</td>";
    txtHtml +="<td>"+value.role.roleName+"</td>";
...
function OnDeleteOne(recId)
{
    ...
    //向后台发送服务请求和数据
    $.post("json/authoritySet!delete.action",
        {
            optionCode:"delone",
            id:recId
        },
...
function OnDelete()
{
    ...
    //执行删除（多条记录）
    //向后台发送服务请求和数据
    $.post("json/authoritySet!deleteItems.action",
        {
...
function OnView(recId)
{
    ...
    //向后台发送服务请求和数据
    $.post("json/authoritySet!findItemById.action",
        {
            optionCode:"view",
            id:recId
        },
        function(data){
            if(data.actionStatus == "ok"){
                $("#userName").val(data.authoritySet.userName);
                $("#idcard").val(data.authoritySet.idcard);
                $("#password").val(data.authoritySet.password);
                $("#authority").val(data.authoritySet.authority);
                $("#modaldialog").modal("show");
            }
            else{
                alert(data.msg);
...
function AddNew()
{
	if($("#authority option:selected").text()==""){
		alert("请选择权限!");
		return;}
	if($("#userName").val()==""){
		alert("请输入用户名！");
    ...
    //向后台发送服务请求和数据
    $.post("json/authoritySet!save.action",
        {
            optionCode:"addnew",
            userName:$("#userName").val(),
            password:$("#password").val(),
            authority:$("#authority option:selected").text(),
        },
        function(data){
...
{	
	if($("#authority option:selected").text()==""){
		...
		return;}
    //向后台发送服务请求和数据
    $.post("json/authoritySet!update.action",
        {
            optionCode:"update",
            id:gCurId,
