...
{
    //向后台发送服务请求和数据
    $.post("json/orderList!findDyAll.action",
        {
            optionCode:"getAll"
        },
        function(data){
...
function tableBuild(i,value){
    ...
    txtHtml +="<td>"+value.planBeginDate.replace("T"," ")+"</td>";
    txtHtml +="<td>"+value.planLastDate.replace("T"," ")+"</td>";
...
{	
	...
	$.post("json/orderList!setCurrentOrder.action",
		...
		function(data){
			...
			//2-启动详情页面
			window.location.href="left.action?val=dystafforderinfo";				
		});	
}
